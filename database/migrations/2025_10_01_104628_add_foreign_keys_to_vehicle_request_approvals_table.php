...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('vehicle_request_approvals', function (Blueprint $table) {
            // Ensure column types match (int unsigned for establishments.e_id; skip if already correct)
            if (Schema::getColumnType('vehicle_request_approvals', 'initiate_establishment_id') !== 'int unsigned') {
                $table->integer('initiate_establishment_id')->nullable()->change();
            }
            if (Schema::getColumnType('vehicle_request_approvals', 'current_establishment_id') !== 'int unsigned') {
                $table->integer('current_establishment_id')->nullable()->change();
            }
            ...
            // Add FK for initiate_establishment_id -> establishments.e_id (int)
            if (!str_contains($tableDefinition, 'initiate_establishment_id_foreign')) {
            ...
            if (!str_contains($tableDefinition, 'current_establishment_id_foreign')) {
                $table->foreign('current_establishment_id', 'current_establishment_id_foreign')
                      ->references('e_id')->on('establishments')
                      ->onDelete('cascade');
            }

            // Add FK for user_id -> users.id (assuming bigint unsigned)
            if (!str_contains($tableDefinition, 'vehicle_request_approvals_user_id_foreign')) {
            ...
            if (!str_contains($tableDefinition, 'vehicle_request_approvals_approved_by_foreign')) {
