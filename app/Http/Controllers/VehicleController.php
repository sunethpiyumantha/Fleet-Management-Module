    ...
    public function update(Request $request, $serialNumber)
    {
        ...
        if ($request->hasFile('image_rear')) {
            if ($vehicle->image_rear) {
                Storage::disk('public')->delete($vehicle->image_rear);
            }
            $data['image_rear'] = $request->file('image_rear')->store('vehicle_images', 'public');
        }
        if ($request->hasFile('image_side')) {
            if ($vehicle->image_side) {
                Storage::disk('public')->delete($vehicle->image_side);
            }
            $data['image_side'] = $request->file('image_side')->store('vehicle_images', 'public');
        }
        ...
        return redirect()->route('vehicles.create')->with('success', 'Vehicle updated successfully.');
    }
