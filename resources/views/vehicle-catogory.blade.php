...
<div style="max-width: 64rem; margin: 0 auto; padding: 2.5rem 1.5rem;">
  <div style="background-color: white; border: 1px solid #f97316; border-radius: 1rem; box-shadow: 0 10px 15px -3px rgba(0,0,0,0.1), 0 4px 6px -2px rgba(0,0,0,0.05); padding: 1.5rem;">
      ...
      <div style="display: flex; flex-direction: column; gap: 1rem; align-items: center;" class="md:flex-row">
        ...
        <div style="width: 100%; max-width: 25%; margin-top: 1rem;" class="md:mt-0">
          <button type="submit"
            ...
            onmouseover="this.style.backgroundColor='#f97316'" onmouseout="this.style.backgroundColor='#10b981'">
    ...
    <div style="margin-bottom: 1rem; display: flex; justify-content: flex-start; align-items: center; gap: 0.5rem;">
      <input type="text" id="searchInput" placeholder="Search Vehicle Category..."
    ...
    <div style="overflow-x: auto;">
      <table id="categoryTable"
        ...
        <tbody id="tableBody">
          @foreach (['Light Vehicle', 'Motor Vehicle', 'Heavy Vehicle', 'Off-Road Vehicle'] as $category)
            <tr>
              <td style="padding: 0.75rem; border-bottom: 1px solid #f3f4f6;">{{ $category }}</td>
...
<script>
  ...
  function renderPagination(totalRows) {
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      btn.textContent = i;
      btn.style = "margin: 0 0.25rem; padding: 0.25rem 0.75rem; background: #ea580c; color: white; border: none; border-radius: 0.375rem; cursor: pointer;";
      if (i === currentPage) {
