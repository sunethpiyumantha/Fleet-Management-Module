...
@section('title', 'Establishment Management')
...
<div style="max-width: 64rem; margin: 0 auto; padding: 2.5rem 1.5rem;">
  <div style="background-color: white; border: 1px solid #f97316; border-radius: 1rem; box-shadow: 0 10px 15px -3px rgba(0,0,0,0.1), 0 4px 6px -2px rgba(0,0,0,0.05); padding: 1.5rem;">
    <h2 style="font-size: 1.875rem; font-weight: bold; color: #ea580c; text-align: center; margin-bottom: 1.5rem;">Establishment Management</h2>
    ...
    <!-- Form -->
    <form class="mb-8" style="margin-bottom: 2rem;" action="{{ route('establishments.store') }}" method="POST">
      @csrf
      <div style="display: flex; flex-direction: column; gap: 1rem; align-items: center;" class="md:flex-row">
        <div style="width: 100%; max-width: 75%;">
          <label for="establishment" style="display: block; margin-bottom: 0.25rem; font-size: 0.875rem; font-weight: 500;">Enter Establishment</label>
          <input type="text" id="establishment" name="name" required
    ...
    <!-- Search Bar -->
    <form method="GET" action="{{ route('establishments.index') }}" style="margin-bottom: 1rem; display: flex; justify-content: flex-start; align-items: center; gap: 0.5rem;">
      <input type="text" name="search" id="searchInput" placeholder="Search Establishment..." value="{{ request('search') }}"
    ...
    <div style="overflow-x: auto;">
      <table id="vehicleTable"
        ...
        <thead style="background-color: #f97316; color: white; cursor: pointer;">
          <tr>
            <th style="padding: 0.75rem;" onclick="sortTable(0)">Establishment ▲▼</th>
        ...
        <tbody id="tableBody">
          @foreach ($establishments as $establishment)
            <tr>
              <td style="padding: 0.75rem; border-bottom: 1px solid #f3f4f6;">{{ $establishment->name }}</td>
              <td style="padding: 0.75rem; text-align: center; border-bottom: 1px solid #f3f4f6;">
                ...
                <form action="{{ route('establishments.update', $establishment->id) }}" method="POST" style="display:inline;">
                  @csrf
                  @method('POST')
                  <input type="text" name="name" value="{{ $establishment->name }}" style="padding: 0.25rem; border-radius: 0.25rem; border: 1px solid #ccc;">
    ...
    <div id="pagination" style="margin-top: 1rem; text-align: center;">
      {{ $establishments->appends(['search' => request('search')])->links() }}
    </div>
  </div>
</div>
