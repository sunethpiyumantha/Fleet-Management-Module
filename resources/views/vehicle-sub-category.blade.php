...
<div style="max-width: 64rem; margin: 0 auto; padding: 2.5rem 1.5rem;">
    <div style="background-color: white; border: 1px solid #f97316; border-radius: 1rem; box-shadow: 0 10px 15px -3px rgba(0,0,0,0.1), 0 4px 6px -2px rgba(0,0,0,0.05); padding: 1.5rem;">
        ...
        <!-- Sub Category Form -->
        <form style="margin-bottom: 2rem; display: flex; flex-direction: column; gap: 1rem; align-items: center;" class="md:flex-row" action="{{ route('vehicle-sub-category.store') }}" method="POST">
            @csrf
            <!-- Category Dropdown -->
                ...
                <thead style="background-color: #f97316; color: white;">
                    <tr>
                        <th style="padding: 0.75rem; cursor: pointer;">
                            <a href="{{ route('vehicle-sub-category.index', ['sort' => 'category', 'order' => request('order') == 'asc' ? 'desc' : 'asc', 'search' => request('search')]) }}"
                               style="text-decoration: none; color: white;">
                                Vehicle Category {{ request('sort') == 'category' ? (request('order') == 'asc' ? '▲' : '▼') : '▲▼' }}
                            </a>
                        </th>
                        <th style="padding: 0.75rem; cursor: pointer;">
                            <a href="{{ route('vehicle-sub-category.index', ['sort' => 'sub_category', 'order' => request('order') == 'asc' ? 'desc' : 'asc', 'search' => request('search')]) }}"
                               style="text-decoration: none; color: white;">
        ...
        <div style="margin-top: 1rem; text-align: center;">
            {{ $subCategories->appends(['search' => request('search'), 'sort' => request('sort'), 'order' => request('order')])->links() }}
        </div>
    </div>
</div>
@endsection