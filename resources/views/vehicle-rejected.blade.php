...
<style>
    ...
    .highlighted-row {
        background-color: #ffebee !important; /* Light red for rejected */
    }
    .search-form {
        margin-bottom: 15px;
    }
...
<div style="width: 100%; padding: 8px; font-family: Arial, sans-serif; background-color: white;">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
        <nav style="font-size: 14px;">
            ...
            <span style="font-weight: bold; color:#023E8A;">Rejected Vehicles</span>
        </nav>
    </div>
    ...
    <!-- Search Form -->
    <form method="GET" action="{{ route('vehicle-rejected.index') }}" class="search-form">
        <input type="text" name="search" placeholder="Search by serial, type, category, etc..." value="{{ request('search') }}">
        <button type="submit">Search</button>
        @if(request('search'))
            <a href="{{ route('vehicle-rejected.index') }}" style="margin-left: 10px; color: #0077B6;">Clear</a>
        @endif
    </form>
    ...
    <table id="vehicleTable" style="width: 100%; border-collapse: collapse; border: 1px solid #90E0EF;">
        ...
        <tbody id="tableBody">
            ...
            @empty
                <tr>
                    <td colspan="13" style="border: 1px solid #90E0EF; padding: 20px; text-align: center; color: #666;">No rejected requests found.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
