...
<div style="width: 100%; padding: 8px; font-family: Arial, sans-serif; background-color: white;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
        <nav style="font-size: 14px;">
            ...
            <span style="font-weight: bold; color:#023E8A;">Vehicle Model</span>
        </nav>
    </div>
    ...
    <form action="{{ route('vehicle-models.store') }}" method="POST" style="margin-bottom: 20px;">
        @csrf
        <div style="display: flex; flex-wrap: wrap; gap: 15px;">
            <div style="flex: 3; min-width: 220px;">
                <label for="vehicleModel" style="display: block; font-size: 14px; margin-bottom: 4px; color:#023E8A;">Enter Vehicle Model</label>
                <input type="text" id="vehicleModel" name="model" value="{{ old('model') }}" required
    ...
    <!-- Search Bar -->
    <form method="GET" action="{{ route('vehicle-models.index') }}" style="margin-bottom: 15px; display: flex; gap: 10px; align-items: center;">
        <input type="text" name="search" id="searchInput" placeholder="Search Vehicle Model..."
    ...
    <div style="overflow-x: auto;">
        <table id="vehicleModelTable" style="width: 100%; border-collapse: collapse; margin-bottom: 15px; font-size: 14px; border: 1px solid #90E0EF;">
            <thead style="background: #023E8A; color: white; text-align: left;">
                <tr>
                    <th style="border: 1px solid #90E0EF; padding: 8px; width: 50px;">#</th>
                    <th style="border: 1px solid #90E0EF; padding: 8px; cursor: pointer;" onclick="sortTable(1)">Vehicle Model ▲▼</th>
            ...
            <tbody id="tableBody">
                @foreach ($models as $index => $model)
                    <tr style="background-color:white; color:#03045E;">
                        ...
                        <td style="border: 1px solid #90E0EF; padding: 8px;">{{ $model->model }}</td>
                        <td style="border: 1px solid #90E0EF; padding: 8px; text-align: center;">
                            ...
                            <form action="{{ route('vehicle-models.destroy', $model->id) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" onclick="return confirm('Delete this Vehicle Model?')"
