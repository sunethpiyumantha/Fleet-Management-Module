...
<div style="max-width: 64rem; margin: 0 auto; padding: 2.5rem 1.5rem;">
  <div style="background-color: white; border: 1px solid #f97316; border-radius: 1rem; box-shadow: 0 10px 15px -3px rgba(0,0,0,0.1), 0 4px 6px -2px rgba(0,0,0,0.05); padding: 1.5rem;">
    <h2 style="font-size: 1.875rem; font-weight: bold; color: #ea580c; text-align: center; margin-bottom: 1.5rem;">Vehicle Model Management</h2>
    ...
    <!-- Add Model Form -->
    <form class="mb-8" style="margin-bottom: 2rem;" action="{{ route('vehicle-models.store') }}" method="POST">
      @csrf
    ...
    <!-- Search Bar -->
    <form method="GET" action="{{ route('vehicle-models.index') }}" style="margin-bottom: 1rem; display: flex; justify-content: flex-start; align-items: center; gap: 0.5rem;">
      <input type="text" name="search" id="searchInput" placeholder="Search Vehicle Model..." value="{{ request('search') }}"
      ...
      <table id="vehicleModelTable"
        ...
        <tbody id="tableBody">
          @foreach ($models as $model)
            <tr>
              <td style="padding: 0.75rem; border-bottom: 1px solid #f3f4f6;">{{ $model->model }}</td>
              <td style="padding: 0.75rem; text-align: center; border-bottom: 1px solid #f3f4f6;">
                <!-- Update -->
                <form action="{{ route('vehicle-models.update', $model->id) }}" method="POST" style="display:inline;">
                  @csrf
                  @method('POST')
                  <input type="text" name="model" value="{{ $model->model }}" style="padding: 0.25rem; border-radius: 0.25rem; border: 1px solid #ccc;">
                ...
                <form action="{{ route('vehicle-models.destroy', $model->id) }}" method="POST" style="display:inline;">
                  @csrf
                  @method('DELETE')
                  <button type="submit" onclick="return confirm('Delete this Vehicle Model?')" style="background-color: #dc2626; color: white; padding: 0.25rem 0.75rem; border-radius: 0.375rem; border: none; margin-left: 0.5rem;">Delete</button>
                </form>
              </td>
            </tr>
          @endforeach
    ...
    <div id="pagination" style="margin-top: 1rem; text-align: center;">
      {{ $models->appends(['search' => request('search')])->links() }}
    </div>
  </div>
</div>
