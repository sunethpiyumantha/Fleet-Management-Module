...
<div style="max-width: 64rem; margin: 0 auto; padding: 2.5rem 1.5rem;">
    <div style="background-color: white; border: 1px solid #f97316; border-radius: 1rem; box-shadow: 0 10px 15px -3px rgba(0,0,0,0.1), 0 4px 6px -2px rgba(0,0,0,0.05); padding: 1.5rem;">
        ...
        <!-- Form -->
        <form class="mb-8" style="margin-bottom: 2rem;" action="{{ route('vehicle-category.store') }}" method="POST">
            @csrf
            <div style="display: flex; flex-direction: column; gap: 1rem; align-items: center;" class="md:flex-row">
                <div style="width: 100%; max-width: 75%;">
                    <label for="vehicleCategory" style="display: block; margin-bottom: 0.25rem; font-size: 0.875rem; font-weight: 500;">Enter Vehicle Category</label>
                    <input type="text" id="vehicleCategory" name="category" required value="{{ old('category') }}"
        ...
        <!-- Search Bar -->
        <form method="GET" action="{{ route('vehicle-category.index') }}" style="margin-bottom: 1rem; display: flex; justify-content: flex-start; align-items: center; gap: 0.5rem;">
            <input type="text" name="search" id="searchInput" placeholder="Search Vehicle Category..." value="{{ request('search') }}"
            ...
            <table id="categoryTable"
                ...
                <tbody id="tableBody">
                    @foreach ($categories as $category)
                        <tr>
                            <td style="padding: 0.75rem; border-bottom: 1px solid #f3f4f6;">{{ $category->category }}</td>
                            <td style="padding: 0.75rem; text-align: center; border-bottom: 1px solid #f3f4f6;">
                                ...
                                <form action="{{ route('vehicle-category.update', $category->id) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('POST')
                                    <input type="text" name="category" value="{{ $category->category }}" style="padding: 0.25rem; border-radius: 0.25rem; border: 1px solid #ccc;">
                                ...
                                <form action="{{ route('vehicle-category.destroy', $category->id) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" onclick="return confirm('Delete this Vehicle Category?')" style="background-color: #dc2626; color: white; padding: 0.25rem 0.75rem; border-radius: 0.375rem; border: none; margin-left: 0.5rem;">Delete</button>
                                </form>
                            </td>
                        </tr>
                    @endforeach
