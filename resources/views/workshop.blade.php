...
<div style="width: 100%; padding: 8px; font-family: Arial, sans-serif; background-color: white;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
        <nav style="font-size: 14px;">
            ...
            <span style="font-weight: bold; color:#023E8A;">Workshop Type</span>
        </nav>
    </div>
    ...
    <!-- Add Workshop Type Form -->
    <form action="{{ route('workshops.store') }}" method="POST" style="margin-bottom: 20px;">
        @csrf
        <div style="display: flex; flex-wrap: wrap; gap: 15px;">
            <div style="flex: 3; min-width: 220px;">
                <label for="workshopType" style="display: block; font-size: 14px; margin-bottom: 4px; color:#023E8A;">Enter Workshop Type</label>
                <input type="text" id="workshopType" name="workshop_type" value="{{ old('workshop_type') }}" required
    ...
    <!-- Search Bar -->
    <form method="GET" action="{{ route('workshops.index') }}" style="margin-bottom: 15px; display: flex; gap: 10px; align-items: center;">
        <input type="text" name="search" id="searchInput" placeholder="Search Workshop Type..."
        ...
        <table id="vehicleTable" style="width: 100%; border-collapse: collapse; margin-bottom: 15px; font-size: 14px; border: 1px solid #90E0EF;">
            <thead style="background: #023E8A; color: white; text-align: left;">
                <tr>
                    ...
                    <th style="border: 1px solid #90E0EF; padding: 8px; cursor: pointer;" onclick="sortTable(1)">Workshop Type ▲▼</th>
            ...
            <tbody id="tableBody">
                @foreach ($workshops as $index => $workshop)
                    <tr style="background-color:white; color:#03045E;">
                        ...
                        <td style="border: 1px solid #90E0EF; padding: 8px;">{{ $workshop->workshop_type }}</td>
                        <td style="border: 1px solid #90E0EF; padding: 8px; text-align: center;">
                            ...
                            <form action="{{ route('workshops.update', $workshop->id) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('POST')
                                <input type="text" name="workshop_type" value="{{ $workshop->workshop_type }}"
                            ...
                            <form action="{{ route('workshops.destroy', $workshop->id) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" onclick="return confirm('Delete this Workshop Type?')"
