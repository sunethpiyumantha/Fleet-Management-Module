...
@section('title', 'Vehicle Engine Capacity Management')
...
<div style="width: 100%; padding: 8px; font-family: Arial, sans-serif; background-color: white;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
        <nav style="font-size: 14px;">
            ...
            <span style="font-weight: bold; color:#023E8A;">Vehicle Engine Capacity</span>
        </nav>
    </div>
    ...
    <!-- Add Vehicle Engine Capacity Form -->
    <form action="{{ route('vehicle-engine-capacity.store') }}" method="POST" style="margin-bottom: 20px;">
        @csrf
        <div style="display: flex; flex-wrap: wrap; gap: 15px;">
            <div style="flex: 3; min-width: 220px;">
                <label for="vehicleType" style="display: block; font-size: 14px; margin-bottom: 4px; color:#023E8A;">Enter Vehicle Engine Capacity</label>
                <input type="text" id="vehicleType" name="engine_capacity" value="{{ old('engine_capacity') }}" required
    ...
    <!-- Search Bar -->
    <form method="GET" action="{{ route('vehicle-engine-capacity.index') }}" style="margin-bottom: 15px; display: flex; gap: 10px; align-items: center;">
        <input type="text" name="search" id="searchInput" placeholder="Search Vehicle Engine Capacity..."
        ...
        <table id="vehicleTable" style="width: 100%; border-collapse: collapse; margin-bottom: 15px; font-size: 14px; border: 1px solid #90E0EF;">
            <thead style="background: #023E8A; color: white; text-align: left;">
                <tr>
                    ...
                    <th style="border: 1px solid #90E0EF; padding: 8px; cursor: pointer;" onclick="sortTable(1)">Engine Capacity ▲▼</th>
            ...
            <tbody id="tableBody">
                @foreach ($capacities as $index => $capacity)
                    <tr style="background-color:white; color:#03045E;">
                        ...
                        <td style="border: 1px solid #90E0EF; padding: 8px;">{{ $capacity->engine_capacity }}</td>
                        <td style="border: 1px solid #90E0EF; padding: 8px; text-align: center;">
                            ...
                            <form action="{{ url('vehicle-engine-capacity/update/' . $capacity->id) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('POST')
                                <input type="text" name="engine_capacity" value="{{ $capacity->engine_capacity }}"
                            ...
                            <form action="{{ route('vehicle-engine-capacity.destroy', $capacity->id) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" onclick="return confirm('Delete this Engine Capacity?')"
